...
def remove_listing(real_estate_list: Dict[str, Dict[str, Any]], name: str) -> Dict[str, Dict[str, Any]]:
    ...
    real_estate_list.pop(name, None)# Rimuove l'elemento se esiste, altrimenti non fa nulla
...
def format_list(real_estate_list: Dict[str, Dict[str, Any]]) -> str:
    ...
    for name, details in real_estate_list.items():
        prezzo = details.get("prezzo", "N/D") # recupera il valore se presente, altrimenti "N/D"
        posizione = details.get("posizione", "N/D") 
        descrizione = details.get("descrizione", "")
        line = f"{name}: {prezzo}, {posizione}, {descrizione}\n"
        lines.append(line)
